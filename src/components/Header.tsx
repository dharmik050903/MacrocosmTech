...
const Header = () => {
  ...
  return (
    <header
      className={`fixed top-2 sm:top-4 left-1/2 -translate-x-1/2 z-[100] w-[95%] sm:w-[90%] md:w-[85%] lg:w-[80%] max-w-[1400px] flex items-center justify-between transition-all duration-300 rounded-xl sm:rounded-[20px] ${
        scrolled
          ? 'bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-xl border border-gray-200/30 dark:border-[#282e39]/30 shadow-2xl py-3 sm:py-4 md:py-6'
          : 'bg-background-light/60 dark:bg-background-dark/60 backdrop-blur-sm border border-gray-200/20 dark:border-[#282e39]/20 shadow-md py-3.5 sm:py-4 md:py-7'
      }`}
    >
      <div className="w-full px-3 sm:px-4 md:px-8 lg:px-10 flex items-center justify-between gap-2 sm:gap-4">
        <Link to="/" className="flex items-center gap-2 sm:gap-3 group min-w-0 flex-shrink">
          <div className="size-7 sm:size-8 flex items-center justify-center rounded-lg bg-primary/10 dark:bg-primary/20 group-hover:bg-primary/20 dark:group-hover:bg-primary/30 transition-colors flex-shrink-0">
            <Logo />
          </div>
          <div className="flex flex-col min-w-0">
            <h2 className="text-slate-900 dark:text-white text-xs sm:text-sm md:text-base lg:text-lg font-bold leading-tight tracking-[-0.015em] group-hover:text-primary dark:group-hover:text-primary transition-colors truncate">
              <span className="hidden sm:inline">MacrocosmTech</span>
              <span className="sm:hidden">Macrocosm</span>
            </h2>
          </div>
        </Link>
        ...
        <button
          ...
          className="md:hidden flex cursor-pointer items-center justify-center overflow-hidden rounded-lg h-9 sm:h-10 w-9 sm:w-10 bg-slate-200 dark:bg-[#282e39] text-slate-900 dark:text-white hover:bg-slate-300 dark:hover:bg-[#1e2533] transition-all active:scale-95 flex-shrink-0"
